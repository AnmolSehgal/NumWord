let tensMultiples:string[],onesPlace:string[],tensPlace:string[];
...
let onLoad:()=>void= function(){
    document.getElementById('showWord').addEventListener('keyup',function():void{
            const srchBar = document.getElementById('showWord') as HTMLInputElement;
            const display = document.getElementById('displayWords');
            display.innerHTML= valueLoader(srchBar.value);
    })
}

let valueLoader:(value:string)=>string = function(val:string){
    let inputValSplitArray:string[] = val.split('');
    if(Number(inputValSplitArray.join(""))===0)
        return onesPlace[0];
    return NumberToWord(inputValSplitArray);
}

let NumberToWord:(value:string[])=> string = function(val:string[]):string{
...
let places:(value:number)=> string = function(val : number):string{
    if(val>7)
        return 'Crore';
    else if(val<=7 && val>5)
        return 'Lac';
